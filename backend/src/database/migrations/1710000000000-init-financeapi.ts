...
export class InitFinanceapi1710000000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE TYPE transaction_status AS ENUM ('ACTIVE','CANCELLED')`);
    ...
    await queryRunner.query(`CREATE TABLE holdings (id SERIAL PRIMARY KEY, portfolio_id INT NOT NULL REFERENCES portfolios(id) ON DELETE CASCADE, symbol VARCHAR NOT NULL, quantity DECIMAL(18,6) NOT NULL, avg_cost DECIMAL(18,4) NOT NULL, current_price DECIMAL(18,4) DEFAULT 0, pnl DECIMAL(18,2) DEFAULT 0, initial_quantity DECIMAL(18,6) DEFAULT 0, initial_avg_cost DECIMAL(18,4) DEFAULT 0)`);
    await queryRunner.query(`CREATE TABLE transactions (id SERIAL PRIMARY KEY, holding_id INT NOT NULL REFERENCES holdings(id) ON DELETE CASCADE, type transaction_type NOT NULL, quantity DECIMAL(18,6) NOT NULL, price DECIMAL(18,4) NOT NULL, fee DECIMAL(18,2) DEFAULT 0, executed_at TIMESTAMP NOT NULL, status transaction_status NOT NULL DEFAULT 'ACTIVE', cancelled_at TIMESTAMP NULL)`);
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP TYPE IF EXISTS portfolio_type, risk_level, transaction_type, transaction_status, asset_status, user_role`);
  }
}
